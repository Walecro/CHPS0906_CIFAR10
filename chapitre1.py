...
class my_class(nn.Module):
    def __init__(self,first,second):
        ...
        self.a = nn.Linear(3*32*32,first)
        self.b = nn.Linear(first,second)
        self.c = nn.Linear(second,nclasses)
...
def fit_one_cycle(epoch,data_train_load,data_valid_load,myclass):
    ...
    opt = torch.optim.SGD(myclass.parameters() ,lr=learningRate)
    ...
    for _ in range(epoch):
        ...
        print(f"Accuracy at epoch n°{_} is {acc_cum/len(iter_valid)} loss is {loss}")

def main():
#Init du modèle 
    myclass = my_class(32,24)
    ...
    data_train_load = torch.utils.data.DataLoader(train_set,batch_size=32,shuffle=True)
    data_valid_load = torch.utils.data.DataLoader(valid_set,batch_size=32,shuffle=True )
